...
async function postAuthDualPath(body, fingerprint, proxy, preferredHost = null, auth1Token = null) {
  ...
  const extraHeaders = { 'Connect-Protocol-Version': '1' };
  if (auth1Token) extraHeaders['X-Devin-Auth1-Token'] = auth1Token;
  const headers = buildJsonHeaders(fingerprint, body, extraHeaders);
...
async function windsurfLoginViaAuth1(email, password, fingerprint, proxy) {
  ...
  const { res: br, label: bl } = await postAuthDualPath(bridgeBody, fingerprint, proxy, null, auth1Token);
