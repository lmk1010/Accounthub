...
import { parseOAuthCredentialRef } from './oauth-credentials.js';
...
export const PROVIDER_MAPPINGS = [
    ...
    },
    {
        // Warp AI OAuth 配置
        dirName: 'warp',
        patterns: ['configs/warp/', '/warp/'],
        providerType: 'claude-warp-oauth',
        credPathKey: 'WARP_OAUTH_CREDS_FILE_PATH',
        defaultCheckModel: 'warp-ai',
        displayName: 'Warp AI OAuth',
        needsProjectId: false,
        urlKeys: ['WARP_BASE_URL', 'WARP_TOKEN_URL']
    },
    {
        // Gemini CLI OAuth 配置
        dirName: 'gemini',
    ...
    },
    {
        ...
        defaultCheckModel: 'gpt-5.3-codex',
    ...
    },
    {
        // xAI Grok OAuth 配置
        dirName: 'xai',
        patterns: ['configs/xai/', '/xai/', 'configs/grok/', '/grok/'],
        providerType: 'openai-xai-oauth',
        credPathKey: 'XAI_OAUTH_CREDS_FILE_PATH',
        defaultCheckModel: 'grok-4.5',
        displayName: 'xAI Grok OAuth',
        needsProjectId: false,
        urlKeys: ['XAI_BASE_URL', 'XAI_CHAT_BASE_URL']
    },
    {
    ...
    {
        // Claude Antigravity OAuth 配置（复用 Antigravity OAuth 流程，独立凭证）
        dirName: 'claude-antigravity',
        patterns: ['configs/claude-antigravity/', '/claude-antigravity/'],
        providerType: 'claude-antigravity',
        credPathKey: 'ANTIGRAVITY_OAUTH_CREDS_FILE_PATH',
        defaultCheckModel: 'claude-sonnet-4-5',
        displayName: 'Claude Antigravity',
        needsProjectId: true,
        urlKeys: []
    },
    {
    ...
    },
    {
        // AMI OAuth 配置
        dirName: 'ami',
        patterns: ['configs/ami/', '/ami/'],
        providerType: 'claude-ami-oauth',
        credPathKey: 'AMI_WOS_SESSION',
        defaultCheckModel: 'claude-opus-4-5',
        displayName: 'Claude AMI OAuth',
        needsProjectId: false,
        urlKeys: []
    },
    {
        // Droid OAuth 配置（OpenAI Chat）
        dirName: 'droid',
        patterns: ['configs/droid/', '/droid/'],
        providerType: 'openai-droid',
        credentialProviderType: 'droid-oauth',
        credPathKey: 'DROID_OAUTH_CREDS_FILE_PATH',
        defaultCheckModel: null,
        displayName: 'Droid OpenAI',
        needsProjectId: false,
        autoLink: false
    },
    {
        // Droid OAuth 配置（OpenAI Responses）
        dirName: 'droid-responses',
        patterns: ['configs/droid-responses/', 'configs/droid-openai-responses/', '/droid-responses/'],
        providerType: 'openaiResponses-droid',
        credentialProviderType: 'droid-oauth',
        credPathKey: 'DROID_OAUTH_CREDS_FILE_PATH',
        defaultCheckModel: null,
        displayName: 'Droid OpenAI Responses',
        needsProjectId: false,
        autoLink: false
    },
    {
        // Droid OAuth 配置（Claude）
        dirName: 'droid-claude',
        patterns: ['configs/droid-claude/', '/droid-claude/'],
        providerType: 'claude-droid',
        credentialProviderType: 'droid-oauth',
        credPathKey: 'DROID_OAUTH_CREDS_FILE_PATH',
        defaultCheckModel: null,
        displayName: 'Droid Claude',
        needsProjectId: false,
        autoLink: false
    }
];
...
export function formatSystemPath(relativePath) {
    ...
    if (relativePath.startsWith('db://')) return relativePath;
...
export function detectProviderFromPath(normalizedPath) {
    const parsedRef = parseOAuthCredentialRef(normalizedPath);
    if (parsedRef) {
        const mapping = PROVIDER_MAPPINGS.find(m => (m.credentialProviderType || m.providerType) === parsedRef.providerType);
        if (mapping) {
            return {
 ...
 */
export async function isValidOAuthCredentials(credentials) {
    try {
        const jsonData = typeof credentials === 'string' ? JSON.parse(credentials) : credentials;
...
export function createProviderConfig(options) {
    const { credPathKey, credPath, defaultCheckModel, needsProjectId, urlKeys, credentialId, checkHealthDefault } = options;
    const defaultHealthCheck = typeof checkHealthDefault === 'boolean'
        ? checkHealthDefault
        : (credPathKey === 'KIRO_OAUTH_CREDS_FILE_PATH');
    ...
    const newProvider = {
        ...
        uuid: generateUUID(),
        oauthCredentialId: credentialId || null,
