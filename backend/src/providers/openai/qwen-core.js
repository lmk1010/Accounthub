...
import { extractCredentialId, parseOAuthCredentialRef } from '../../utils/oauth-credentials.js';
import { updateCredentialsById } from '../../services/oauth-credentials-store.js';
...
export class QwenApiService {
    constructor(config) {
        ...
        this.oauthCredentialId = extractCredentialId(config, 'QWEN_OAUTH_CREDS_FILE_PATH');
        this.tokenManagerOptions = { credentialId: this.oauthCredentialId, providerType: 'openai-qwen-oauth' };
        ...
        try {
            ...
            const credentials = await new Promise((resolve, reject) => {
                const checkInterval = setInterval(async () => {
                    try {
                        const record = await oauthCredentialsDao.findLatestByProviderType('openai-qwen-oauth', startTime);
                        if (record && record.credentials?.access_token) {
                            clearInterval(checkInterval);
                            this.oauthCredentialId = record.id;
                            this.tokenManagerOptions.credentialId = record.id;
    ...
    async _loadCachedQwenCredentials(client) {
        ...
        try {
            const record = await oauthCredentialsDao.findById(this.oauthCredentialId);
    ...
    async _cacheQwenCredentials(credentials) {
        ...
        try {
            await updateCredentialsById(this.oauthCredentialId, credentials);
            console.log(`[Qwen Auth] Credentials cached to database (${this.oauthCredentialId})`);
        } catch (error) {
            console.error(`[Qwen Auth] Failed to cache credentials: ${error.message}`);
        }
    }
...
class SharedTokenManager {
    ...
    resolveCredentialId(value) {
        if (!value) return null;
        if (typeof value === 'number') return value;
        if (typeof value === 'string') {
            const parsed = parseOAuthCredentialRef(value);
    ...
    getContext(options = {}) {
        const credentialId = this.resolveCredentialId(options.credentialId || options.credentialFilePath);
        if (!credentialId) {
            throw new TokenManagerError(TokenError.FILE_ACCESS_ERROR, 'OAuth credential id is required');
        }
        let context = this.contexts.get(credentialId);
        if (!context) {
            context = {
                credentialId,
                providerType: options.providerType || null,
                memoryCache: { credentials: null, updatedAt: 0, lastCheck: 0 },
                refreshPromise: null,
            };
    ...
    async checkAndReloadIfNeeded(context) {
        ...
        const record = await oauthCredentialsDao.findById(context.credentialId);
        if (!record || !record.credentials) {
            context.memoryCache.credentials = null;
            context.memoryCache.updatedAt = 0;
            return;
        }

        const updatedAt = record.updated_at ? new Date(record.updated_at).getTime() : now;
        if (updatedAt > (context.memoryCache.updatedAt || 0)) {
            context.memoryCache.credentials = record.credentials;
            context.memoryCache.updatedAt = updatedAt;
        }
    }
    ...
    async performTokenRefresh(context, qwenClient, forceRefresh = false) {
        ...
        const dedupeKey = `qwen-token-refresh:${context.credentialId}`;

        try {
            const credentials = await withDeduplication(dedupeKey, async () => {
                return await withDbLock(dedupeKey, 10, async () => {
                    await this.checkAndReloadIfNeeded(context);
                    ...
                    await updateCredentialsById(context.credentialId, newCredentials);
                    context.memoryCache.updatedAt = Date.now();
            ...
            if (error instanceof CredentialsClearRequiredError) {
                await updateCredentialsById(context.credentialId, {});
                throw error;
            }
