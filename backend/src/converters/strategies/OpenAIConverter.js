...
function normalizeOpenAIResponsesTool(tool) {
    ...
    if (definition.strict !== undefined) normalized.strict = definition.strict;
    delete normalized.function;
    return normalized.name ? normalized : null;
}

function normalizeOpenAIResponsesToolChoice(toolChoice) {
...
function normalizeResponsesTextContent(content, role = 'user') {
    ...
    return content
        .map(part => {
            ...
            if (part.type === 'input_text' || part.type === 'output_text' || part.type === 'text') {
                const text = safeString(part.text ?? part.content);
                return text ? { ...part, type: textType, text } : null;
            }
            if (part.type === 'image_url' || part.type === 'input_image') {
                const image = part.image_url;
                const imageUrl = typeof image === 'string' ? image : safeString(image?.url);
                if (!imageUrl) return null;
                const normalized = {
                    type: 'input_image',
                    image_url: imageUrl
                };
                const detail = part.detail ?? image?.detail;
...
function normalizeResponsesReasoningItem(item) {
    ...
    if (!Array.isArray(normalized.summary)) {
        const summaryText = safeString(item.text ?? item.reasoning_content ?? item.reasoning_text ?? item.content);
        if (summaryText) {
...
function extractOpenAIMessageReasoningItems(message = {}) {
    const items = [];
    const add = (candidate) => {
        ...
        const normalized = normalizeResponsesReasoningItem(candidate);
    ...
    return items.filter(item => {
        const key = item.id || item.encrypted_content || JSON.stringify(item.summary || []);
...
function openAIMessageToResponsesInputItems(message) {
    ...
    if (role === 'tool') {
        return [{
            ...
            call_id: message.tool_call_id || message.id || `call_${uuidv4().replace(/-/g, '')}`,
            output: message.content == null ? '' : safeJsonStringify(message.content)
        }];
    }

    const items = [];
    if (role === 'assistant') {
        items.push(...extractOpenAIMessageReasoningItems(message));
    }

    items.push({
        type: 'message',
        role,
        content: normalizeResponsesTextContent(message.content, role)
    });
    ...
    if (role === 'assistant') {
        for (const toolCall of toolCalls) {
            ...
            items.push({
                type: 'function_call',
                call_id: toolCall.id || `call_${uuidv4().replace(/-/g, '')}`,
                name,
...
export class OpenAIConverter extends BaseConverter {
    ...
    toClaudeResponse(openaiResponse, model) {
        ...
        return {
            ...
            stop_sequence: null,
            usage: (() => {
                const usage = extractTokenUsage(openaiResponse.usage || {});
                return {
    ...
    toClaudeStreamChunk(openaiChunk, model) {
        ...
        if (typeof openaiChunk === 'object' && !Array.isArray(openaiChunk)) {
            ...
            if (finishReason) {
                ...
                // 发送 message_delta
                const streamUsage = extractTokenUsage(openaiChunk.usage || {});
                events.push({
                    type: "message_delta",
                    delta: {
                        stop_reason: stopReason,
                    ...
                    usage: {
                        input_tokens: streamUsage.uncachedInputTokens,
                        cache_creation_input_tokens: streamUsage.cacheCreationTokens,
                        cache_read_input_tokens: streamUsage.cacheReadTokens,
    ...
    toOpenAIResponsesRequest(openaiRequest) {
        ...
        if (openaiRequest.messages && openaiRequest.messages.length > 0) {
            responsesRequest.input = openaiRequest.messages.flatMap(msg => openAIMessageToResponsesInputItems(msg));
        }

        if (openaiRequest.reasoning !== undefined) {
            responsesRequest.reasoning = openaiRequest.reasoning;
        } else if (openaiRequest.reasoning_effort !== undefined || openaiRequest.reasoningEffort !== undefined) {
            responsesRequest.reasoning = {
                effort: String(openaiRequest.reasoning_effort ?? openaiRequest.reasoningEffort).trim().toLowerCase()
            };
        }
        ...
        const maxOutputTokens = openaiRequest.max_output_tokens
        ...
        if (Array.isArray(openaiRequest.tools)) {
            responsesRequest.tools = openaiRequest.tools
                .map(normalizeOpenAIResponsesTool)
                .filter(Boolean);
        }
        if (openaiRequest.tool_choice !== undefined) {
            responsesRequest.tool_choice = normalizeOpenAIResponsesToolChoice(openaiRequest.tool_choice);
        }
        if (openaiRequest.parallel_tool_calls !== undefined) {
            responsesRequest.parallel_tool_calls = openaiRequest.parallel_tool_calls;
        }
        if (typeof openaiRequest.prompt_cache_key === 'string' && openaiRequest.prompt_cache_key.trim()) {
            responsesRequest.prompt_cache_key = openaiRequest.prompt_cache_key.trim();
        }
        if (openaiRequest.prompt_cache_retention !== undefined) {
            responsesRequest.prompt_cache_retention = openaiRequest.prompt_cache_retention;
        }
    ...
    toOpenAIResponsesResponse(openaiResponse, model) {
        ...
        output.push(...extractOpenAIMessageReasoningItems(message));

        // 构建message输出
        const messageContent = normalizeResponsesTextContent(message.content, 'assistant');
        ...
        for (const toolCall of toolCalls) {
            ...
            if (!name) continue;
            const args = toolCall.function?.arguments ?? toolCall.arguments ?? '';
            const callId = toolCall.id || `call_${uuidv4().replace(/-/g, '')}`;
            output.push({
                type: 'function_call',
                id: callId,
                call_id: callId,
