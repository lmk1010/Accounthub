...
import { batchImportKiroRefreshTokensStream, importAwsCredentials, stopKiroPollingTask, stopAllKiroPollingTasks } from '../../auth/oauth-handlers.js';
...
export async function handlePotluckApiRoutes(method, path, req, res) {
    ...
    try {
        ...
        if (method === 'POST' && path === '/api/potluck/keys/apply-bonus') {
            const allKeyIds = await getAllKeyIds();
            let totalSynced = 0;
            ...
            for (const apiKey of allKeyIds) {
                try {
                    // 获取用户凭据并检查健康状态
                    const credentials = await getUserCredentials(apiKey);
...
export async function handlePotluckUserApiRoutes(method, path, req, res) {
    ...
    try {
        ...
        if (method === 'GET' && path === '/api/potluckuser/usage') {
            ...
            // 获取资源包详情
            const bonusDetails = await getBonusDetails(apiKey);
        ...
        // POST /api/potluckuser/kiro/cancel-polling - 取消 Kiro OAuth 轮询任务
        if (method === 'POST' && path === '/api/potluckuser/kiro/cancel-polling') {
            const body = await parseRequestBody(req);
            const { taskId } = body;
            ...
            if (taskId) {
                // 取消指定任务
                const stopped = stopKiroPollingTask(taskId);
                result = { success: true, stopped: stopped ? 1 : 0, taskId };
            } else {
                // 取消所有 Kiro 轮询任务
                const count = stopAllKiroPollingTasks();
                result = { success: true, stopped: count };
            }
...
async function handleKiroImportAwsCredentials(req, res, apiKey) {
    try {
        ...
        console.log(`[API Potluck User] Starting AWS credentials import (user: ${apiKey.substring(0, 12)}..., poolId: ${poolId})`);

        const result = await importAwsCredentials(credentials, false, poolId);
...
function resolveCredentialId(credential) {
    ...
    if (credential.oauthCredentialId) return Number(credential.oauthCredentialId);
    if (credential.oauth_credential_id) return Number(credential.oauth_credential_id);
    const rawPath = credential.path;
    if (typeof rawPath === 'number') return rawPath;
    if (typeof rawPath !== 'string') return null;
    const parsed = parseOAuthCredentialRef(rawPath);
    if (parsed) return parsed.id;
    if (/^\d+$/.test(rawPath)) return Number(rawPath);
    return null;
}

async function syncCredentialHealthFromPool(apiKey, credential) {
    const credentialId = resolveCredentialId(credential);
    if (!credentialId) {
        return { isHealthy: false, message: '仅支持数据库凭据' };
    }

    // 从 ProviderPoolManager 获取该凭据对应的 provider 状态
    const poolManager = getProviderPoolManager();
    if (poolManager && credential.provider) {
        const providerPool = await providerDao.findAll(credential.provider, { includeDeleted: true });
        if (providerPool.length > 0) {
            const matchedProvider = providerPool.find((provider) => {
                const providerOauthId = provider.oauth_credential_id
                    || provider.credentials?.oauthCredentialId
                    || provider.credentials?.oauth_credential_id;
                if (providerOauthId && Number(providerOauthId) === credentialId) {
                    return true;
                }
                const ref = provider.credentials?.kiroOAuthCredsFile || provider.credentials?.oauthCredsFile;
                return ref && ref === credential.path;
            });

            if (matchedProvider) {
                const isHealthy = matchedProvider.is_healthy && !matchedProvider.is_disabled;
                let message = '健康检查:正常';

                if (matchedProvider.is_disabled) {
                    message = '已禁用';
                } else if (!matchedProvider.is_healthy) {
                    message = matchedProvider.last_error_message || '健康检查:异常';
                }
    ...
    if (!record) {
        return { isHealthy: false, message: '凭据不存在' };
    }

    const expiresAt = record.credentials?.expiresAt || record.credentials?.expires_at;
    if (expiresAt) {
...
let healthCheckIntervalMs = 5 * 60 * 1000;
...
async function checkAllCredentialsHealth() {
    const allUsers = await getAllUsersCredentials();
 ...
 * 启动定时健康检查
 */
export function startHealthCheckScheduler(options = {}) {
    if (healthCheckTimer) {
        clearInterval(healthCheckTimer);
    }

    const intervalMinutes = Number.isFinite(options.intervalMinutes) && options.intervalMinutes > 0
        ? options.intervalMinutes
        : (CONFIG.POTLUCK_HEALTH_CHECK_INTERVAL_MINUTES ?? 5);
    healthCheckIntervalMs = intervalMinutes * 60 * 1000;
    const initialDelayMs = Number.isFinite(options.initialDelayMs) && options.initialDelayMs >= 0
    ...
    console.log(`[API Potluck] Health sync scheduler started (interval: ${healthCheckIntervalMs / 1000}s)`);
}
