 ...
 */
async function loadTokenFromStore(credentialId) {
    if (!credentialId) {
        return null;
    }
    ...
    const json = record.credentials;
    const refreshToken = json.refreshToken || json.refresh_token || '';
    console.log(`[iFlow] Token loaded from database (${credentialId}) (refresh_token: ${refreshToken ? refreshToken.substring(0, 8) + '...' : 'EMPTY'})`);
 ...
 */
async function saveTokenToStore(credentialId, tokenStorage) {
    if (!credentialId) {
        throw new Error('[iFlow] credentialId is required to save token');
    }
    ...
    await updateCredentialsById(credentialId, json);
    console.log(`[iFlow] Token saved to database (${credentialId}) (refresh_token: ${json.refresh_token ? json.refresh_token.substring(0, 8) + '...' : 'EMPTY'})`);
}
...
export class IFlowApiService {
    constructor(config) {
        ...
        this.oauthCredentialId = extractCredentialId(config, 'IFLOW_TOKEN_FILE_PATH');
        this.isInitialized = false;
        this.tokenStorage = null;
        
        // 配置 HTTP/HTTPS agent，优化高并发性能
    ...
    async initializeAuth(forceRefresh = false) {
        ...
        if (!this.oauthCredentialId) {
            throw new Error('[iFlow] OAuth credential id is required.');
        }
        
        try {
            this.tokenStorage = await loadTokenFromStore(this.oauthCredentialId);
            if (this.tokenStorage && this.tokenStorage.apiKey) {
                this.apiKey = this.tokenStorage.apiKey;
                console.log('[iFlow Auth] Authentication configured successfully from database.');
                ...
                throw new Error('[iFlow] Token record does not contain a valid API key.');
            }
        } catch (error) {
            console.error('[iFlow Auth] Error initializing authentication:', error.code || error.message);
            console.error('[iFlow Auth] Failed to initialize authentication from database:', error.message);
    ...
    async _checkAndRefreshTokenIfNeeded() {
        ...
        try {
            ...
            const dedupeKey = `iflow-token-refresh:${this.oauthCredentialId || 'unknown'}`;
            ...
            if (this.isExpiryDateNear()) {
                const refreshedStorage = await loadTokenFromStore(this.oauthCredentialId);
    ...
    async _refreshOAuthTokens() {
        ...
        // 保存到数据库
        await saveTokenToStore(this.oauthCredentialId, this.tokenStorage);
