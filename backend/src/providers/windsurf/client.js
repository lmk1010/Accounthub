...
function extractCompactSystemFacts(sysText) {
  const facts = [];
  // Path segment: Unix /~ or Windows C:\ — captures until whitespace/quotes/special chars
  const pathSeg = String.raw`([/~A-Za-z](?:[^\s\`'"<>\n,;)]|\\(?=[^\s]))+)`;
  const patterns = [
    // "CWD: /path" — simple inline form (CLAUDE_CODE_SIMPLE mode)
    [new RegExp(String.raw`(?:^|\n)\s*CWD:\s*` + pathSeg, 'i'), 'Working directory'],
    // "current working directory: /path"
    [new RegExp(String.raw`current working directory(?:\s+is)?\s*[:=]?\s*\`?` + pathSeg, 'i'), 'Working directory'],
    // "Primary working directory: /path" — computeSimpleEnvInfo (new format)
    [new RegExp(String.raw`(?:^|\n)\s*(?:[-*]\s+)?Primary working directory\s*[:=]\s*\`?` + pathSeg, 'i'), 'Working directory'],
    // "Working directory: /path" — <env> block (old format)
    [new RegExp(String.raw`(?:^|\n)\s*(?:[-*]\s+)?Working directory\s*[:=]\s*\`?` + pathSeg, 'i'), 'Working directory'],
    // git status — both formats
    [/(?:^|\n)\s*(?:[-*]\s+)?Is a git repository\s*[:=]\s*([^\n<]+)/i, 'Git repo'],
...
/** Extract the user's real CWD from a raw or compacted system prompt. */
function extractCwdFromText(text) {
  if (!text) return null;
  const pathSeg = String.raw`([/~A-Za-z][^\s\`'"<>\n,;)]+)`;
  const patterns = [
    new RegExp(String.raw`(?:^|\n)\s*CWD:\s*` + pathSeg, 'i'),
    new RegExp(String.raw`(?:^|\n)\s*(?:[-*]\s+)?Primary working directory\s*[:=]\s*\`?` + pathSeg, 'i'),
    new RegExp(String.raw`(?:^|\n)\s*(?:[-*]\s+)?Working directory\s*[:=]\s*\`?` + pathSeg, 'i'),
    new RegExp(String.raw`current working directory(?:\s+is)?\s*[:=]?\s*\`?` + pathSeg, 'i'),
  ];
...
/** Append CWD override to already-processed text, sourcing CWD from raw original. */
function appendCwdOverride(processed, rawForCwdExtraction) {
  const cwd = extractCwdFromText(rawForCwdExtraction);
  if (!cwd) return processed;
  return processed + '\n\n' + buildCwdOverride(cwd);
}

/** Build a strong CWD override instruction to anchor all tool call paths. */
function buildCwdOverride(cwd) {
  return `IMPORTANT — USER WORKING DIRECTORY: The user's real project files are at \`${cwd}\`.` +
    ` ALL file paths in tool calls (bash, read_file, write_file, grep, glob, etc.) MUST be absolute paths rooted at this directory.` +
    ` Do NOT use any server-side workspace path. Treat \`${cwd}\` as your working directory for every operation.`;
}

export function compactSystemPromptForCascade(sysText) {
  if (!sysText) return sysText;
  const stripped = sysText.replace(/^x-anthropic-billing-header:[^\n]*(?:\n|$)/gmi, '').trim();
  if (process.env.CASCADE_COMPACT_CLAUDE_SYSTEM === '0') return appendCwdOverride(neutralizeIdentityForCascade(stripped), stripped);
  ...
  if (!looksLikeClaudeCode || stripped.length < 4000) {
    return appendCwdOverride(neutralizeIdentityForCascade(stripped), stripped);
  }
  ...
  const cwd = extractCwdFromText(stripped);
  if (cwd) lines.push('', buildCwdOverride(cwd));
  return lines.join('\n');
}
  ...
  async cascadeChat(messages, modelEnum, modelUid, opts = {}) {
    ...
    try {
      ...
      try {
        ...
        log.info(`[Usage-diag] GeneratorMetadata raw bytes=${metaResp?.length ?? 0} hex=${metaResp?.slice(0,32).toString('hex')}`);
        serverUsage = parseGeneratorMetadata(metaResp);
        log.info(`[Usage-diag] parseGeneratorMetadata → ${serverUsage ? `in=${serverUsage.inputTokens} out=${serverUsage.outputTokens} cache_r=${serverUsage.cacheReadTokens} cache_w=${serverUsage.cacheWriteTokens}` : 'null'}`);
      } catch (e) {
        log.info(`[Usage-diag] GetCascadeTrajectoryGeneratorMetadata FAILED: ${e.message}`);
      }
