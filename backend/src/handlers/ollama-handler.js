 ...
 */
export async function getProviderByModelName(modelName, providerPoolManager, defaultProvider) {
    if (!modelName) {
    ...
    const hasHealthyProvider = (providers = []) =>
        providers.some(p => p.is_healthy && !p.is_disabled && !p.is_deleted);
    ...
    if (lowerModelName.includes('claude') || lowerModelName.includes('sonnet') || lowerModelName.includes('opus') || lowerModelName.includes('haiku')) {
        for (const [providerType, providers] of Object.entries(providersByType)) {
            if (providerType.includes('claude') || providerType.includes('kiro')) {
                if (hasHealthyProvider(providers)) {
                    return providerType;
                }
            }
        }
...
export async function handleOllamaTags(req, res, apiService, currentConfig, providerPoolManager) {
    try {
        ...
        if (providerPoolManager) {
            let pools = null;
            if (typeof providerPoolManager.getProviderPools === 'function') {
                pools = await providerPoolManager.getProviderPools();
            }
            if (!pools) {
                pools = {};
                const allProviders = await providerDao.findAll(null, { includeDeleted: true });
            ...
            // Phase 2+3: 只挑本 shard 拥有的健康 provider,避免跨 shard 创建 adapter
            let shardMod = null;
            try { shardMod = await import('../utils/shard.js'); } catch (_e) { /* ignore */ }

            for (const [providerType, providers] of Object.entries(pools)) {
                // Skip if already processed
                if (processedProviderTypes.has(providerType)) continue;

                const healthyProvider = providers.find(p => {
                    if (!p.is_healthy || p.is_disabled) return false;
                    if (shardMod?.SHARD_ENABLED && !shardMod.ownsUuid(p.uuid)) return false;
                    return true;
                });
                if (healthyProvider) {
...
export async function handleOllamaChat(req, res, apiService, currentConfig, providerPoolManager) {
    try {
        ...
        if (!apiService || detectedProvider !== currentConfig.MODEL_PROVIDER) {
            if (providerPoolManager) {
                // Select provider from pool (lock-free atomic counter)
