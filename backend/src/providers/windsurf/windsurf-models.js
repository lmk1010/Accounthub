...
/**
 * Reverse-lookup: given a modelUid string (e.g. 'MODEL_PRIVATE_2' or 'claude-sonnet-4-6'),
 * return the internal model key (e.g. 'claude-4.5-sonnet').
 * Falls back to resolveModel(uid) so plain internal keys pass through unchanged.
 */
export function resolveModelByUid(uid) {
  if (!uid) return null;
  for (const [key, info] of Object.entries(MODELS)) {
    if (info.modelUid === uid) return key;
  }
  return resolveModel(uid);
}
