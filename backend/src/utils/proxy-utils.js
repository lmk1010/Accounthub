...
function normalizeProxyInput(proxyUrl) {
...
export function parseProxyUrl(proxyUrl) {
    ...
    const normalizedProxyUrl = normalizeProxyInput(proxyUrl);
    ...
    try {
        ...
        if (protocol === 'socks5:' || protocol === 'socks4:' || protocol === 'socks:') {
            // SOCKS 代理
            const socksAgent = new SocksProxyAgent(normalizedProxyUrl);
            return {
            ...
            // HTTP/HTTPS 代理
            return {
                httpAgent: new HttpProxyAgent(normalizedProxyUrl),
                httpsAgent: new HttpsProxyAgent(normalizedProxyUrl),
                proxyType: 'http'
            };
        } else {
...
export function getProxyConfigForProvider(config, providerType) {
    if (config?.proxyAgent) {
        const proxyNodeLabel = config?.proxyNode?.name || config?.proxyNode?.id || 'pool-node';
        console.log(`[Proxy] Using pool proxy for ${providerType}: ${proxyNodeLabel}`);
        return {
            httpAgent: config.proxyAgent,
            httpsAgent: config.proxyAgent,
            proxyType: 'pool'
        };
    }
...
export function configureAxiosProxy(axiosConfig, config, providerType) {
    if (config?.proxyAgent) {
        axiosConfig.httpAgent = config.proxyAgent;
        axiosConfig.httpsAgent = config.proxyAgent;
