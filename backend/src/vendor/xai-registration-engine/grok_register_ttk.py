...
DEFAULT_CONFIG = {
    ...
    "hotmail_imap_fallback": True,
    "hotmail_graph_last_n": 30,
...
# 其中 Token 为 Microsoft OAuth2 refresh_token，验证码优先通过 Graph Mail.Read 拉取。
...
def hotmail_refresh_access_token(account, log_callback=None, purpose="graph"):
    ...
    with lock:
        ...
        endpoints = (
            HOTMAIL_GRAPH_TOKEN_ENDPOINTS
            if purpose == "graph"
            else HOTMAIL_IMAP_TOKEN_ENDPOINTS
        )
        for url, extra in endpoints:
            try:
                ...
                if access_token:
                    ...
                    if log_callback:
                        api_name = "Graph" if purpose == "graph" else "IMAP"
                        log_callback(
                            f"[*] Hotmail OAuth2 {api_name} access_token 刷新成功: {email_addr}"
                        )
                    return access_token
                ...
                last_error = exc
                continue
        raise Exception(f"Hotmail OAuth2 {purpose} refresh 失败: {last_error}")


def _hotmail_graph_recipient_blob(message):
    values = []
    ...
    for header in message.get("internetMessageHeaders") or []:
        ...
        if name in tracked_headers:
            values.append(str(header.get("value") or ""))
    return " ".join(values).lower()


def _hotmail_graph_get_code(
    mailbox_email,
    target_email,
    access_token,
    log_callback=None,
    ...
    try:
        ...
        recent_seconds = 900
    try:
        last_n = int(config.get("hotmail_graph_last_n", 30) or 30)
    except Exception:
        last_n = 30
    ...
    if log_callback:
        log_callback(f"[Debug] Hotmail/Outlook Graph 查询: user={mailbox_email}")
    resp = http_get(
    ...
    if resp.status_code >= 400:
        try:
            ...
            error_message = None
        raise Exception(
            f"Graph HTTP {resp.status_code}: {error_message or resp.text[:200]}"
        )

    try:
        payload = resp.json()
    except Exception as exc:
        raise Exception(f"Graph 邮件响应不是有效 JSON: {exc}") from exc
    messages = payload.get("value") if isinstance(payload, dict) else None
    if not isinstance(messages, list):
        raise Exception("Graph 邮件响应缺少 value 列表")
    ...
    for message in messages:
        ...
        if received_at:
            try:
                ...
                if int(received_dt.timestamp() * 1000) < filter_after_ts:
                    continue
            except Exception:
                pass
        ...
        sender = " ".join(
            str(sender_info.get(key) or "") for key in ("name", "address")
        ).strip()
        recipient_blob = _hotmail_graph_recipient_blob(message)
        ...
        if not any(keyword in combined_lower for keyword in keywords):
            continue
        ...
        if require_recipient and target_lower and target_lower not in recipient_blob:
            ...
            if detail_resp.status_code >= 400:
                if detail_resp.status_code in (401, 403):
                    raise Exception(f"Graph HTTP {detail_resp.status_code}: 邮件头读取失败")
                continue
            try:
                detail = detail_resp.json()
            except Exception:
                continue
            recipient_blob = _hotmail_graph_recipient_blob(detail)
            if target_lower not in recipient_blob:
                continue
            combined = f"{combined}\n{recipient_blob}"
...
def hotmail_get_oai_code(
    ...
    imap_fallback = _config_bool(
        config.get("hotmail_imap_fallback", True), default=True
    )
    next_resend_at = time.time() + 60
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if code:
                return code
            if log_callback:
                log_callback(f"[Debug] Hotmail/Outlook Graph 本轮未找到验证码: {email}")
        except Exception as exc:
            graph_access_token = None
            graph_error = exc
            if log_callback:
                log_callback(f"[Debug] Hotmail/Outlook Graph 拉取失败: {exc}")

        if graph_error is not None and imap_fallback:
            try:
                ...
                code = None
                host_errors = []
                imap_hosts = _hotmail_get_imap_hosts()
                for imap_host in imap_hosts:
                    try:
                        ...
                        if log_callback:
                            log_callback(
                                f"[Debug] Hotmail/Outlook IMAP 回退 host 失败: "
                                f"{imap_host}: {host_exc}"
                            )
                if code is None and host_errors and len(host_errors) >= len(imap_hosts):
                ...
                imap_access_token = None
                if log_callback:
                    log_callback(f"[Debug] Hotmail/Outlook IMAP 回退失败: {exc}")
        sleep_with_cancel(current_interval, cancel_callback)
...
const codeInput = Array.from(document.querySelectorAll('input')).find((node) => {
    ...
    return (
        ...
        meta.includes('验证') || meta.includes('one-time') || inputMode === 'numeric' ||
...
return 'enter';
    ...
    if last_snapshot:
        ...
        raise Exception(
            f"未找到邮箱输入框或注册按钮，最后页面: "
            f"url={url}; inputs={inputs or 'none'}; buttons={buttons or 'none'}"
        )
...
- 成功、失败、当前运行占用的 alias 都会去重，并优先通过 Microsoft Graph Mail.Read 收验证码
- Graph 授权不可用时可回退 outlook.office365.com XOAUTH2 IMAP
