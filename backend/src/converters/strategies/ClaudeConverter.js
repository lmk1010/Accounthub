...
function normalizeClaudeToolChoiceForOpenAI(toolChoice) {
...
export class ClaudeConverter extends BaseConverter {
    ...
    toOpenAIRequest(claudeRequest) {
        ...
        if (claudeRequest.messages && Array.isArray(claudeRequest.messages)) {
            ...
            for (const msg of claudeRequest.messages) {
                ...
                if (role === "user" && Array.isArray(msg.content)) {
                    let bufferedContent = [];
                    const flushBufferedUser = () => {
                        if (bufferedContent.length === 0) return;
                        const contentConverted = this.processClaudeContentToOpenAIContent(bufferedContent);
                        const normalized = this._normalizeOpenAIContent(contentConverted);
                        if (normalized && (Array.isArray(normalized) ? normalized.length > 0 : String(normalized).trim().length > 0)) {
                    ...
                    for (const item of msg.content) {
                        if (item && typeof item === 'object' && item.type === "tool_result") {
                            ...
                            const contentStr = this._stringifyClaudeToolResultContent(item.content);
                            tempOpenAIMessages.push({
                                role: "tool",
                ...
                // 处理 assistant 消息中的工具调用（支持多个 tool_use）
                if (role === "assistant" && Array.isArray(msg.content)) {
                    ...
                    for (const part of msg.content) {
                        if (part && typeof part === 'object' && part.type === "tool_use") {
                            const funcName = part.name || "";
                            const funcArgs = part.input || {};
                            toolCalls.push({
                                id: part.id || `call_${funcName}_${toolCalls.length + 1}`,
                                type: "function",
                                function: {
                                    name: funcName,
                                    arguments: JSON.stringify(funcArgs)
                                },
                                index: part.index || toolCalls.length
                            });
                            continue;
                        }

                        if (part && typeof part === 'object' && part.type === 'image' && part.source?.type === 'base64') {
                            contentParts.push({
                                type: 'image_url',
                                image_url: {
                    ...
                    const normalized = this._normalizeOpenAIContent(contentParts);
                    ...
                    } else if (normalized && (Array.isArray(normalized) ? normalized.length > 0 : String(normalized).trim().length > 0)) {
                        tempOpenAIMessages.push({
                            role: "assistant",
                            content: normalized
                        });
                ...
                // 普通文本消息
                const contentConverted = Array.isArray(msg.content)
                    ? this.processClaudeContentToOpenAIContent(msg.content)
                    : msg.content;
                const normalized = this._normalizeOpenAIContent(contentConverted);
                if (normalized && (Array.isArray(normalized) ? normalized.length > 0 : String(normalized).trim().length > 0)) {
                    tempOpenAIMessages.push({
        ...
        if (claudeRequest.tools) {
            ...
            openaiRequest.tool_choice = normalizeClaudeToolChoiceForOpenAI(claudeRequest.tool_choice);
        }
     ...
     * 处理Claude内容到OpenAI格式
     */
    _normalizeOpenAIContent(content) {
        if (!content) return '';
    ...
    _stringifyClaudeToolResultContent(content) {
    ...
    toOpenAIResponsesRequest(claudeRequest) {
        ...
        return new OpenAIConverter().toOpenAIResponsesRequest(openaiRequest);
    }
