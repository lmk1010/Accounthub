...
export class ProviderStrategy {
    ...
    /**
     * Updates the system prompt in MySQL database.
     * NOTE: Disabled auto-update to prevent client system prompt from overwriting
     * user-configured rules in database (e.g., <output-limits>).
     * Database system_prompt_content should only be updated via UI/API config.
    ...
    async _updateSystemPromptFile(incomingSystemText, providerName) {
        // Disabled: Do not auto-update database with client system prompt
        // This was causing user-configured rules to be overwritten
        // Database system_prompt_content should only be managed via UI config
        return;
    }
}
